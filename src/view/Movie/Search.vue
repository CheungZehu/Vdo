	...
	<div id="search">
		<mu-paper class="demo-paper" :zDepth="1">
			<mu-appbar title="Title" class="search-bar">
				...
				<input v-model.trim="q" :value="text" @keyup.enter="search" autofocus="autofocus" class="bar-text" hintText="请输入关键字" />
				<mu-icon-button class="bar-icon" icon="close" slot="right" @click="colse()" />
			</mu-appbar>
		</mu-paper>
			...
			<router-link :to="{name: 'MovieSubject', params: {id: subject.id}}" v-for="subject in subjects" :key="subject.id">
				<mu-paper class="elem" :zDepth="1">
					...
					<div class="text">
						...
						<div class="star-list">
							<span class="star-bor" v-if="subject.rating.average > 0">
						...
						<div class="genres">类型：{{subject.genres | string}}</div>
						<div class="director">导演：{{subject.directors.map(e=>e.name) | string}}</div>
						<div class="cast">主演：{{subject.casts.map(e=>e.name) | string}}</div>
					</div>
				</mu-paper>
			</router-link>
		</div>
	...
	export default {
		...
		methods: {
			...
			gotoTab() {
				...
				console.log(-this.$store.state.movie.searchStep)
          ...
          <router-link :to="{name: 'MovieSubject', params: {id: subject.id}}" v-for="subject in subjects">
            <mu-paper class="elem" :zDepth="1">
                ...
                <div class="text">
                    ...
                    <div class="genres">类型：{{subject.genres}}</div>
                    <div class="director">导演：{{subject.directors.map(e=>e.name)}}</div>
                    <div class="cast">主演：{{subject.casts.map(e=>e.name)}}</div>
                </div>
            </mu-paper>
          </router-link>
        </div>
